...
- local-only policy declarations that report dependency/build path categories in status and Agent Run metadata while untracked machine-local directories are not copied into prepared agent workspaces.
...
Local-only path policy coverage declares `node_modules` as `dependency` and `dist` as `build`, checks `status --json` and Agent Run Contract metadata, and proves untracked dependency content from the source checkout is absent from the prepared agent clone.
