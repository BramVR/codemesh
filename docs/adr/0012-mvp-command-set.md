...
CodeMesh v0 ships `init`, `scan`, `add`, `tree`, `status`, `hydrate`, `bootstrap`, `target export`, `env bind`, `machine register`, `agent prepare`, `agent run`, `runs`, and `clean`. These commands prove the canonical workspace index, local machine identity, manifest topology bootstrap, target-ready export contracts, private fake-provider env binding, and the agent handoff workflow without a daemon, mount layer, automatic placeholders, live secret providers, cloud sync, UI, or file-level lazy hydration.
